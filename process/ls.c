     ...
     char **argv;
{
  ...
  program_name = argv[0];
// i is the index of the first non-option argument
// See the comment of extern int optind (line 50 of getopt_core.h) for explanation
// Basically decode_switches() calls getopt(),
// which uses opind, an extern int to tell successive calls of getopt
// how much of ARGV has been scanned so far
  ...
  nfiles = 100;
// A modern xmalloc implementation
// https://github.com/gcc-mirror/gcc/blob/master/libiberty/xmalloc.c
  files = (struct file *) xmalloc (sizeof (struct file) * nfiles);
  files_index = 0;

// It's a bunch of free() for files[i].name and files[i].linkname
// Since files_index is 0 the function doesn't do anything
// Not sure why it is here 
  clear_files ();

// Why set dir_defaulted to 0 when i < argc?
// When i < argc, it means there are 1+ non-option argument at the end
// e.g. ls -a -l /dev /etc
// argc =5 while i=3, index of /dev
// This means that we are not listing the working directory,
// which should be ls -a -l
// By definition, dir_defaulted = 1 if we are listing working directory
// and dir_defaulted = 0 if not, thus we set it to 0 when i < argc
  if (i < argc)
    dir_defaulted = 0;
// Add all non-option args (in the case of ls, files/folders)
// to the current table of files
  for (; i < argc; i++)
// int gobble_file (name, explicit_arg, dirname)
// This passes 1 to explicit_arg in gobble_file();
// This passes "" to dirname in gobble_file();
    gobble_file (argv[i], 1, "");

// From above, dir_defaulted = 0 if i < argc, so this must be i >= argc
// which means we are listing working directory such as ls -a -l
// and we all know that working directory is the dot .
  if (dir_defaulted)
    {
      if (immediate_dirs)
	gobble_file (".", 1, "");
...
// I think this is similar to gobble_file() but for directories
// It's using a linked list (see function for details)
	queue_directory (".", 0);
    }

// Recall that gobble_file() increments files_index
  if (files_index)
    {
// Setting up sort type and invoking qsort
      sort_files ();
      if (!immediate_dirs)
...
// This is to get terminal size (and set line_length accordingly)
#ifdef TIOCGWINSZ
  {
    struct winsize ws;
...
// Parse options, anything followed by :, such as "w:", needs an argument
...
// From man page (NOTE THAT man page is post glibc 2 so this might be wrong):
...
// check long option struct to see why 10 is sort
// argmatch() returns -1 when no match has been found
// otherwise returns the index of abbreviated match (doesn't have to be exact match)
	case 10:		/* +sort */
...
// Basically it generates a file struct object and initialize the values
...
// gobble_file can be called in a loop to gobble multiple files/folders
// if reaches upper limit, realloc double space
...
// I think index 0~(files_index-1) are occupied
// So this seems to be initializing the first unused one
...
// Recall that name is argv[i], the first non-option arg, so it's either a file or a folder name
// Recall that the dirname is the third arg passed, so if you passed "" dirname[0]==0 is true
      if (name[0] == '/' || dirname[0] == 0)
	path = name;
      else
// TODO: I don't quite understand this else block, need to check code that actually call this

	{
// I think the extra 2 chars are for '/' and '\0'
...
// If we want to trace links to pointed objects
	...
	{
// man lstat
// put the info about path into stat
// returns -1 if error
...
// If it's not a real file/folder, maybe it's a symbol link
// stat the link itself, not the object it points to
	val = lstat (path, &files[files_index].stat);
// If still doesn't work, gives up and set exit_status to 1
      if (val < 0)
	{
...
// defined in stat.h,
// S_ISLNK is a macro that is used to check whether a file is a symbolic link or not
#ifdef S_ISLNK
...
// S_ISDIR(mode) checks whether it's a directory
// This is also defined in stat.h
// unsigned int mode
// In struct stat, we have __mode_t st_mode; also defined in stat.h
// while __mode_t is a defined number
...
// convert_blocks() is defined in the same 1991 library
// in lib/fsusage.c
